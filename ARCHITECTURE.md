...
|---|---|---|---|
| Language | Python 3.11+ | Go, TypeScript, Kotlin | JD requirement. Python async is mature; FastAPI + Pydantic is best-in-class. |
...
| GraphQL | **Strawberry** | Graphene, Ariadne, Gqlgen | JD requirement. Strawberry is code-first, typed (dataclasses), async-native, supports DataLoader / extensions / subscriptions natively. Graphene is sync-first. Ariadne is schema-first (ok but duplicates types). |
...
| Object storage | GCS (MinIO locally) | S3, Azure Blob | JD = GCP. MinIO is S3-compatible so code is portable. |
...
| IaC | **Pulumi Python** | Terraform, CDK for Terraform, Crossplane | JD mentions Pulumi Python specifically. Pulumi writes IaC as real Python (not HCL DSL), so logic can reuse functions, conditionals, loops. Terraform's HCL is fine but less expressive. |
| CI | GitHub Actions | GitLab CI, Cloud Build | JD mentions both GH Actions + Cloud Build. GH Actions is more portable across vendors. |
| Observability | OpenTelemetry (vendor-neutral) → Jaeger local / Cloud Trace prod | Datadog, Honeycomb, New Relic | OTel is the CNCF standard; the exporter swaps without changing app code. GCP Cloud Trace + Cloud Logging + Cloud Monitoring = JD-aligned stack. |
